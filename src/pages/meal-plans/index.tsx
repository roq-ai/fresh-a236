...
type ColumnType = ColumnDef<MealPlanInterface, unknown>;

interface MealPlanListPageProps {
...
export function MealPlanListPage(props: MealPlanListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MealPlanInterface>>(
    () => `/meal-plans?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/meal-plans/view/${record.id}`} passHref legacyBehavior>
                <Button
