...
type ColumnType = ColumnDef<CommunicationInterface, unknown>;

interface CommunicationListPageProps {
...
export function CommunicationListPage(props: CommunicationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CommunicationInterface>>(
    () => `/communications?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CommunicationInterface) => {
    if (hasAccess('communication', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/communications/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/communications/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('communication', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/communications/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="communication" />
        </Flex>

        {hasAccess('communication', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/communications/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
