...
import { getMealPlans } from 'apiSdk/meal-plans';
import { MealPlanInterface } from 'interfaces/meal-plan';
import { getNutritionCourses } from 'apiSdk/nutrition-courses';
import { NutritionCourseInterface } from 'interfaces/nutrition-course';
import { getCommunications } from 'apiSdk/communications';
...
function DashboardPage() {
  ...
  const mealPlanStat = useSWR<PaginatedInterface<MealPlanInterface>>(
    () => `/meal-plans`,
    () => getMealPlans({ limit: 1, offset: 0 }),
  );

  const nutritionCourseStat = useSWR<PaginatedInterface<NutritionCourseInterface>>(
    () => `/nutrition-courses`,
    () => getNutritionCourses({ limit: 1, offset: 0 }),
  );

  const communicationStat = useSWR<PaginatedInterface<CommunicationInterface>>(
    () => `/communications`,
    () => getCommunications({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Meal Plans',
      result: mealPlanStat?.data?.totalCount,
      isLoading: mealPlanStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Nutrition Courses',
      result: nutritionCourseStat?.data?.totalCount,
      isLoading: nutritionCourseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Communications',
      result: communicationStat?.data?.totalCount,
      isLoading: communicationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
